...
<template>
	<div>
		<PortalPageHeader :title="`Invoice #${invoice?.invoice_number}`" :breadcrumbs="[
			{
				title: 'Portal',
				href: '/portal',
			},
			...
			<section class="relative flex flex-col gap-8 md:justify-between md:flex-row">
				...
				<div id="company-branding" class="space-y-1.5">
					...
					<p v-if="globals?.phone">
						<UButton :to="createTel(globals.phone)" color="gray" variant="link" :padded="false"
							icon="material-symbols:phone-android-outline-rounded">
							{{ globals.phone }}
						</UButton>
					</p>
					<p v-if="globals?.email">
						<UButton :to="createMailto(globals.email, { subject: `Invoice #${invoice?.invoice_number}` })" color="gray"
							variant="link" :padded="false" icon="material-symbols:alternate-email-rounded">
							{{ globals.email }}
						</UButton>
					</p>
				</div>
